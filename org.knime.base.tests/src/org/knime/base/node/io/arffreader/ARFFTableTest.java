...
public class ARFFTableTest extends TestCase {
    ...
    private static final NodeLogger LOGGER = NodeLogger.getLogger(ARFFTableTest.class);

    private static final String TEMP_FILE_PREFIX = "ARFFReaderUnitTest";
    ...
    /**
     * tests the creation of a table spec from a nice ARFF file.
     *
    ...
    public void testCreateDataTableSpecFromARFFfileFOO() throws IOException, InvalidSettingsException {
        ...
        Path tempFile = Files.createTempFile(TEMP_FILE_PREFIX, "mini");
        try (final Writer out = Files.newBufferedWriter(tempFile)) {
            out.write(ARFF_FOO);
        }
        try {
            ...
            assertEquals(tSpec.getColumnSpec(0).getName(), "col1");
            assertEquals(tSpec.getColumnSpec(1).getName(), "COL1");
            assertEquals(tSpec.getColumnSpec(2).getName(), "col2");
            assertEquals(tSpec.getColumnSpec(3).getName(), "COL2");
            assertEquals(tSpec.getColumnSpec(4).getName(), "col3");
            assertEquals(tSpec.getColumnSpec(5).getName(), "col4");
            assertEquals(tSpec.getColumnSpec(6).getName(), "col5");
            assertEquals(tSpec.getColumnSpec(7).getName(), "col6");
    ...
    public void testCreateDataTableSpecFromARFFfileIRIS() throws IOException, InvalidSettingsException {
        ...
        try {
            ...
            assertEquals(tSpec.getColumnSpec(0).getName(), "sepallength");
            assertEquals(tSpec.getColumnSpec(1).getName(), "sepalwidth");
            assertEquals(tSpec.getColumnSpec(2).getName(), "petallength");
            assertEquals(tSpec.getColumnSpec(3).getName(), "petalwidth");
            assertEquals(tSpec.getColumnSpec(4).getName(), "class");
            assertEquals(tSpec.getColumnSpec(0).getType(), DoubleCell.TYPE);
            assertEquals(tSpec.getColumnSpec(1).getType(), DoubleCell.TYPE);
            assertEquals(tSpec.getColumnSpec(2).getType(), DoubleCell.TYPE);
            assertEquals(tSpec.getColumnSpec(3).getType(), DoubleCell.TYPE);
    ...
    /**
     * test the creation of a table spec from the IRIS data in an ARFF file. (With the nominal value list not separated
     * with a space)
     *
     * @throws IOException if it wants to.
     * @throws InvalidSettingsException if it feels like.
     */
    public void testCreateDataTableSpecFromARFFfileIrisBar() throws IOException, InvalidSettingsException {
    ...
    /**
     * tests stuff like '?' the ARFF missing value, comment lines and empty lines in the file.
     *
    ...
    public void testARFFTableMissVals() throws IOException, InvalidSettingsException {
        ...
        Path tempFile = Files.createTempFile(TEMP_FILE_PREFIX, "table");
        try (final Writer out = Files.newBufferedWriter(tempFile)) {
            out.write(ARFF_FIES);
        }
        try {
            ARFFTable table = new ARFFTable(tempFile.toUri().toURL(),
                ARFFTable.createDataTableSpecFromARFFfile(tempFile.toUri().toURL(), null), "Row");
    ...
    /**
     * Customer file. Weka is able to read it. We failed on the missing space in the last "@attribute" line.
     *
     * @throws IOException some time.
     * @throws InvalidSettingsException sometimes.
     */
    public void testARFFwithMissingSpace() throws IOException, InvalidSettingsException {
        ...
        Path tempFile = Files.createTempFile(TEMP_FILE_PREFIX, "missSpace");
        try (final Writer out = Files.newBufferedWriter(tempFile)) {
