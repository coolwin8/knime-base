 ...
 */
public final class PathRelativizerTableInput implements PathRelativizer {
     ...
     * @param filterMode
     */
    public PathRelativizerTableInput(final boolean includeParentFolder) {
        m_includeParentFolder = includeParentFolder;
    }

    //TODO check if this is really necessary for do we offer includeParentFolder?
    @Override
    public String apply(final Path sourceFilePath) {
        if (Files.isRegularFile(sourceFilePath)) {
            ...
            //TODO check what we want to do
            if (m_includeParentFolder) {
                return sourceFilePath.getParent() == null ? sourceFilePath.getRoot().toString()
                    : sourceFilePath.getParent().relativize(sourceFilePath).toString();
            } else {

                return sourceFilePath.relativize(sourceFilePath).toString();
            }
        }
    }
}
